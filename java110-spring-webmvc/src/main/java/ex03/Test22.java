...
public class Test22 {
    ...
    // 2) HttpMessageConverter 구현체를 못 찾았다면?
    ...
    //      "google-gson" 라이브러리
    //
    // -> 따라서 @RestController를 사용하려면 위의 두 개의 라이브러리 중 하나는 반드시 포함해야 한다
    ...
    //@Restcontroller일 경우 리턴타입이 String 이라면 그대로 출력한다
    @RequestMapping("m4")
  ...
  //@Restcontroller일 경우 리턴타입이 primitive type 이라면 문자열로 변환하여 그대로 출력한다
    @RequestMapping("m5")
    ...
    // 클라이언트가 보낸 JSON데이터를 String 변수에 받기
    // -> JSON 문자열 -> String
    @RequestMapping("m6")
    ...
    // -> 생략하면,
    //    클라이언트가 보낸 데이터가 application/x-www-form-urlencoded 형식이라고 간주한다
    ...
    // 빈 맵 객체를 넘겨 줄 것이다.
    // Map 변수를 JSON 데이터를 받고 싶다면, 변수 앞에 @RequestBody를 명시해야 한다
    // 이 경우 생략하면 안된다.
    @RequestMapping("m9")
