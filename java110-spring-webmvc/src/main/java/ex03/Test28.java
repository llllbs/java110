// 세션 다루기 - @ModelAttribute 애노테이션과 SessionStatus세션
package ex03;
...
public class Test28 {
    ...
    public String m1(Model model, HttpSession session) throws Exception {
        ...
        model.addAttribute("name2","유관순"); // 프론트 컨트롤러가 세션에 보관
        model.addAttribute("gender2","여자"); // 프론트 컨트롤러가 세션에 보관
        
        // 프론트 컨트롤러가 ServletRequest에 값 보관
        // -> @SessionAttributes에 지정된 이름이 아니기 때문이다
        model.addAttribute("tel2","1111-1111"); // 프론트 컨트롤러가 세션에 보관
        
        
        session.setAttribute("age2", "28"); // 페이지 컨트롤러가 직접 세션에 보관
    ...
    public String m2(
            ...
            // -> 만약 @SessionAttributes에 나열된  이름이 아니면,
            //      - 세션에 값이 있을 경우, 빈문자열을 주입해준다.
            //      - 세션에 값이 없을 경우, 빈문자열을 주입해준다.
            ,@ModelAttribute("tel2") String tel2
            
            
            // -> 만약 @SessionAttributes에 지정된 이름의 값이 없다면,
            //    예외가 발생된다.
            , @ModelAttribute("age2") String age2
            // @SessionAttributes -> @ModelAttribute는 한쌍임을 기억
    ...
    @ResponseBody
    public String m4(
            // 다른 페이지 컨트롤러에 @SessionAttributes에서 (test27/m1)
            // 지정된 값을 받고 싶을 때?
    ...
    public String m5(HttpSession session) throws Exception {
        // 다른 페이지 컨트롤러에 @SessionAttributes에서 (test27/m1)
        // 지정된 값을 HttpSession 객체에서  직접 꺼낼때?
        // -> 꺼낼 수 있다
    ...
    public String logout2(SessionStatus status) throws Exception {
        ...
        // 현재 이 request handler가 소속되어 있는 페이지 컨트롤러 에서
...
//    특정 페이지 컨트롤러에서만 사용하고 관리할 값이라면 @SessionAttributes로 등록하라!
//
// -> 값 제거
//    HttpSession.invalidate() 세션을 완전히 무효화시킨다
...
//      작업이 완료되어서 이제는 세션에 잠시 보관된 값들을 제거하고 싶다!"
//    라는 의미다