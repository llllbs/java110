...
public class MyWebApplicationIniializer /*implements WebApplicationInitializer*/ {
    
    
    public void onStartup(ServletContext servletContext) throws ServletException {
        // 구동 원리:
        // 1) 서블릿 컨테이너(예: 톰캣서버)를 실행한다
        // 2) 각각의 웹 애플리케이션의 /WEB-INF/lib폴더에 있는 .jar 파일을 로딩한다
        ...
        //       그 클래스 정보를 목록에 담아 준다
        //    -> SpringServletContainerInitializer의 onStart()에서는
        //       목록에 등록된 클래스의 객체를 한 개씩 만들고,
        //       그 객체에 대해 onStartup()을 호출한다
        //    -> 즉 지금 여기에서 만드는 MyWebApplicationInitializer 객체를 만들고
        //       onStartup()을 호출한다
