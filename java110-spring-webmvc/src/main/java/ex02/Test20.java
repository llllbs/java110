...
public class Test20 {
    ...
    // 테스트:
    //      http://localhost:8888/ex02/Test20.hrml
    // 주의!
    // -> 프론트 컨트롤러가 멀티파트 데이터를 처리하려면,
    // 방법1) DD 파일(web.xml)에 프론트 컨트롤러를 선언할 때 다음의 태그를 설정해야 한다
    //       <multipart-config>
    //          <max-file-size>2000000</max-file-size>
    //       </multipart-config>
    //
    // 방법2) Appache common fileupload를 이용하여 멀티파트 데이터를 처리하는
    //       스프링 제공 객체를 추가하면 된다
    // -> multipartResolve 라는 이름으로 다음과 같이 객체를 등록한다
