...
public class Test15 {
    ...
    // 실행 오류를 발생시킨다
    // 해결책?
    // -> 클라이언트가 보낸 데이터 String 타입 데이터를 메서드의 파라미터 타입으로 변환 시키는
    ...
    //    이 애노테이션이 지정된 메서드를 호출 하여 먼저 초기화시킨 다음에 사용할 것을 요구하는 애노테이션이다.
    // -> 클라이언트가 보낸 데이터를 request handler의 각 파라미터 값으로 바꾸기 전에 이 메서드를 호출
    //    할 것이다.
    //    즉 파라미터 갯수 만큼 호출한다
    @InitBinder
    public void initBinder(WebDataBinder binder) {// 프론트 컨트롤러가 호출
        System.out.println("오호라.. 출력될까!");
        
        // yyyy-MM-dd 형식으로 클라이언트가 보낸 문자열을 java.util.Date 객체로 바꿔주는 도구
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
        format.setLenient(true);// 24시간 표기 방법을 사용할 것인지의 여부
        
        // String -> java.util.Date 변환기 등록
        binder.registerCustomEditor(Date.class
               // , new DatePropertyEditor(format)
                ,new CustomDateEditor(format, true));
        // 개발자가 만든것을 사용해도 되고, 스프링에서 제공하는 것을 써도 된다
