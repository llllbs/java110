...
// -> 의존 객체를 곱을 setter에 이 애노테이션을 붙이면
//    스프링 IoC 컨테이너가 해당 객체를 찾아 자동으로 주입하게 만들 수 있다
//    setter를 호출할 때 해당 객체를 찾아 자동으로 주입한다
//
// -> 단, 스프링 IoC 컨테이너는 기본으로 @Autowired를 처리하지 않는다
//    @Autowired 애노테이션을 처리하려면 도우미 객체를 스프링 IoC 컨테이너에 장착해야 한다
...
//    그냥 등록만 하면 된다. 그러면 스프링 IoC 컨테이너가 알아서 이 객체를 사용하여 @Autowired를 처리한다
//
// BeanPostProcessor?
// -> 스프링 IoC 컨테이너는 객체를 생성한 후 각 객체에 대해 BeanPostProcessor를 실행한다
//    즉 BeanPostProcessor가 있으면 실행하고 없으면 실행하지 않는다
// -> AutowiredAnnotationBeanPostProcessor 클래스는 바로 BeanPostProcessor를 구현한 클래스이다 
package ex08;
