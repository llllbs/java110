...
public class Servlet03 extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        try {
            List<FileItem> items = 
                    upload.parseRequest((HttpServletRequest)req); // 원래 req가 HttpServletRequest

            for(FileItem item: items) {
                if(item.isFormField()) {// 일반데이터
                    parts.put( // 일반데이터
                            item.getFieldName()// 파라미터명
                            ,item.getString("UTF-8"));// 파라미터값
                }else {
                    String filename = UUID.randomUUID().toString();// 새로운 이름으로 저장(기존 파일명에 상관X)
