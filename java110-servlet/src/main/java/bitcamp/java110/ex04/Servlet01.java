/*클라이언트가 보낸 데이터 읽기 - GET 요청으로 본내 데이터
 * 
 */
...
public class Servlet01 extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 테스트:
        // http://localhost:8888/ex04/get.html 페이지에서 값을 입력 후 보내기 버튼
...
// -> 예)
//GET /ex04/servlet01?name=aaa&age=10&working=true HTTP/1.1
//Host: localhost:8888
...
//- 보안
//  -> URL은 브라우저 캐시에 자동으로 보관되기 때문에 노출되지 말아야 할 데이터를 전송할때는 적합하지 않다.
//  -> 예를 들면 로그인, 회원가입 등에는 적합하지 않다.
//- 대용량 데이터
//  -> 대부분의 웹서버는 request-uri 또는 헤더를 포함한 메타 데이터의 크기를 8KB 미만으로 설정하고 있다.
//  -> GET 요청으로는 대용량의 데이터를 전송할 수 없다.
//- 바이너리 데이터 전송
//  -> request-URI에 포함할 수 있는 것은 텍스트 이기 때문에 바이너리 데이터(이미지, 엑셀, 영샹 등 )를 전송하는데 적합하지 않다.
